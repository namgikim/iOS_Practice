...
class TodoViewController: UIViewController {
    ...
    private func initializeViews() {
        ...
        if let todo: Todo = self.todo {
            ...
            self.shouldNotifySwitch.isOn = todo.shouldNotify
            self.mode = Mode.view
        }
    }
    ...
    @objc private func touchUpCancelButton(_ sender: UIBarButtonItem) {
        ...
        if self.todo == nil {
            ...
            /// 그렇지 않다면 다시 원래 todo 상태로 화면을 초기화 해줌
    ...
    @objc private func touchUpDoneButton(_ sender: UIBarButtonItem) {
        ...
        // 새로운 todo 생성
        let todo: Todo
        todo = Todo(title: title, due: self.dueDatePicker.date, memo: self.memoTextVIew.text, shouldNotify: self.shouldNotifySwitch.isOn, id: self.todo?.id ?? String(Date().timeIntervalSince1970)) //유닉스 타임스탬프를 할 일 고유 아이디로 활용
    ...
    override func viewDidLoad() {
        ...
        // 이전 화면에서 전달받은 todo 가 없다면 새로운 작성화면 설정
        if self.todo == nil {
            self.navigationItem.leftBarButtonItem = self.cancelButton
